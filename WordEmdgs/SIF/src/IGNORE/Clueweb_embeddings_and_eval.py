...
def plot(X, y, popup_labels=None, title=None):
...
def t_sne(data, labels, sentences, title=None):
    ...
    print np.shape(X_2d)
    plot(X_2d, labels, popup_labels=sentences, title=title)
...
for weightpara in weightparas:
    ...
    for rmpc in rmpcs:
        ...
        with open('./common_base_triples.txt') as g:
            ### only load textual mentions of triples with the most mentions
            keys = [eval(i.strip()) for i in g.readlines()]
            print keys
            for key in keys:
                data_selected = data[key]
                X, labels, sentences = get_embeddings_clueweb(We, words, data_selected, weight4ind, params)
                t_sne(X, labels, sentences, str(key))
